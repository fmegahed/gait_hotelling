...
names(subgroupList) <- c(paste0("subject", c(1:13, 15)))
...
names(T2Features) <- c(paste0("subject", c(1:13, 15)))
...
```{r T2_plot_acc, fig.align='center', results='asis', out.width="100%"}

load(file="../Data/RGenerated/T2Out_ACC.Rdata")

for (id in setdiff(1:15, c(13))) {
  T2 <- get(paste0("sub", id, "T2"))$T2
  UCL <- get(paste0("sub", id, "T2"))$UCL
  end <- length(T2)
  x2 <- 10 + end
  
  color1 <- ifelse(T2[1:20]>UCL, "OC", "IC")
  T2_df_1 <- data.frame(x=11:30, T2=T2[1:20], shape="Phase 1", color=color1)
  color2 <- ifelse(T2[21:end]>UCL, "OC", "IC")
  T2_df_2 <- data.frame(x=31:x2, T2=T2[21:end], shape="Phase2", color=color2)
  T2_12_df <- rbind(T2_df_1, T2_df_2)
  UCL_df <- data.frame(UCL=UCL, ltype="UCL")
  
  T2_plot <- ggplot(data=T2_12_df) +
    geom_point(aes(x=x, y=T2, shape=shape, col=color), size=2.5) +
    geom_line(aes(x=x, y=T2)) +
    geom_hline(aes(yintercept=UCL, linetype=ltype), data=UCL_df) +
    xlab(element_blank()) + ylab(element_blank()) +
    theme(legend.title=element_blank()) + # This removes the legend title
    theme(legend.position=c(.1, .68)) +
    theme(legend.text = element_text(size=10)) +
    scale_x_continuous(limits = c(0, x2)) +
    scale_shape_manual(values = c(6, 16)) +
    scale_color_manual(values = c("black", "red")) +
    theme(axis.text = element_text(size=15)) +
    ggtitle(paste0("Subject ", id)) +
    theme(plot.title = element_text(size = 15, hjust=0.5, margin = margin(t=10, b=-20))) #+
  # theme(plot.margin = margin(0,0,0,0, "in"))
  
  # ggsave(paste0("T2_acc_sub", id, ".png"), width = 10, height = 5, dpi = "retina", units = "in", device="png", scale = 1, path = "../Data/Figures/")
...
for (id in seq(length(subgroupList))) {
  ...
  T2LHD[[id]] <- list(T2=T2, UCL=UCL)

}
names(T2LHD) <- c(paste0("subject", c(1:13, 15)))
...
load(file="../Data/RGenerated/T2Out_LHD.Rdata")

for (id in setdiff(1:15, c(13))) { #in setdiff(1:15, c(13))) {
  ...
  # ggsave(paste0("Tsq_sub", id, ".png"), width = 10, height = 5, dpi = "retina", units = "in", device="png", scale = 1, path = "../Data/RGenerated/")
