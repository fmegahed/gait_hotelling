...
Using $T^2$ statistic, we compared the mean of subgroups against a reference multivariate distribution by accounting for the correlation between different variables. We calculated the $T^2$ for subgroups using the method introduced in @mason2001applying. Also, to construct a $T^2$ chart, one needs to find the control limits based on the distribution parameters. However, as we investigated the gait data in our analysis, it did not follow a parametric distribution. Therefore, we implemented a non-parametric approach to bootstrap the control limits in both retrospective and prospective phases using the approach in @phaladiganon2011bootstrap.


## $T^2$ Calculations on Gait Acceleration Profiles

Our $T^2$ charting approach is comprised of 3 steps as described below:

1. **phase 0:** The purpose of phase 0 is to construct a clean *in-control* data set that captures the reference distribution. In this phase we remove the outliers in one pass and update the subgroups. Then we calculate the within-batch covariance and the mean of the subgroups that will be used for $T^2$ calculations in the following phases.

2. **phase 1:**

3. **phase 2:**

the reference multivariate distribution is defined in phase 0. This accounts for the correlation between the variables.
Initialize.

In the following sections we present the control charts for retrospective phase (phase0) and the prospective phase (phase 1 & 2).
