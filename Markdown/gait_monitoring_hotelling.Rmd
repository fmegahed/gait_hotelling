...
<span style="color: blue;">-----------The previous chunk ("read"), have to run when the following chunk runs. When I add "dependson="dataCleaning" to the previous chunk ("read"), it always reruns causes knitting to take a long time. ---------------</span>. 
...
for (id in seq(length(subgroupList))) {
  ...
  phase1ExpTimes <- subgroupList[[id]]$LHD$X1 %>% sapply(function(x) {x[,4]}) %>%
    do.call(what=c)
  ...
  # Should the grey profiles include phase0???????????????????????????
  phase1Idx <- match.closest(phase1ExpTimes, expT)
  
  # for (i in which(T2Profiles[[id]]$T2 > T2Profiles[[id]]$UCL)) {
  #   print(i)
  # }
  ...
  plot(fdata(a[phase1Idx,]), col="grey", ylim=c(-5, 40), xlim=c(0, 70),
       xlab="Gait Time Index", ylab="Acceleration Magnitude",
       cex.axis=1.5, cex.lab=1.5, cex.main=1.5,
       main="Profile Visual Inspection")
  lines(fdata(a[OCIdx,]), col="red")
  legend("topleft", legend=c("Phase 1 Profiles", expression("Subgroup with Largest T"^2)),
...
# for (id in 1:2) {
  ...
  for(ii in 1:length(t3)){
    ...
    tmpList[[ii]] <- data.frame(q)
    colnames(tmpList[[ii]]) <- c("decomposition", "decomposition control limit",
...
for (id in seq(length(subgroupList))) {
  ...
  for (i in seq(ncol(featurePair))) {
    ...
    plot(1 ,1, type = "n", xlim = xlim, ylim = ylim, main="Ellipse Chart", cex.main=1.5,
